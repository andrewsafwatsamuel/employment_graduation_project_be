...
def update_company_password_use_case(
        ...
        old_password,
        new_password,
        retrieve_company_operation=retrieve_company_by_id,
        update_password_operation=update_company_password
):
    if company_id is None or not is_valid_string_input(old_password):
        raise Exception("Invalid inputs")
    if not is_valid_string_input(new_password, PASSWORD_REGEX):
        raise Exception("Invalid new password")
    company = retrieve_company_operation(company_id)

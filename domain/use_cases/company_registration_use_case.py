...
def company_registration_use_case(company_db, phone, operation=insert_company):
    ...
    if not is_valid_string_input(phone, EGYPTIAL_PHONE_NUMBER_REGEX):
        raise Exception("You must put at least one phone number")

    hashed_password = generate_password_hash(company_db[COMPANY_PASSWORD])
    company_db.update({COMPANY_PASSWORD: hashed_password})
    return operation(company_db, phone)
